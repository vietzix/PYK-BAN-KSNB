  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Xin chào! Tôi là Trợ lý AI của Ban KSNB PVEP. Tôi đã sẵn sàng hỗ trợ bạn tra cứu dữ liệu phiếu ý kiến. Bạn có thể hỏi tôi về trạng thái một phiếu cụ thể, hoặc nhờ tôi tổng hợp lỗi theo từng lĩnh vực dưới dạng bảng biểu.",
      timestamp: new Date()
    }
  ]);
...
6. Trình bày chuyên nghiệp, súc tích, chuẩn văn phong kiểm soát nội bộ.
      ...
      // gọi API server-side (Vercel) để tránh lộ API key trên trình duyệt
      ...
      // stream nếu server hỗ trợ; nếu không thì đọc JSON
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "Xin lỗi, hệ thống AI đang gặp sự cố kết nối. Vui lòng kiểm tra lại kết nối mạng hoặc thử lại sau ít phút.",
          timestamp: new Date()
        }
      ]);
    } finally {
              ...
              >
                <div className={`flex gap-3 max-w-[92%] ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`space-y-1 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                    <div
                      className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm overflow-x-auto ${
                        msg.role === "user"
              ...
              <button
                ...
                className={`absolute right-2.5 top-1/2 -translate-y-1/2 p-2.5 rounded-xl transition-all ${
                  input.trim() && !isLoading
                    ? "bg-brand-500 text-white shadow-lg shadow-brand-200 hover:scale-105 active:scale-95"
